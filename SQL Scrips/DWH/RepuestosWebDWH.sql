	...
	AS
	    BEGIN
	        ...
	        WHILE @CurrentDate < @EndDate
	            BEGIN
	                IF NOT EXISTS (SELECT DateKey FROM Dimension.Fecha WHERE DateKey = YEAR(@CurrentDate) * 10000 + MONTH(@CurrentDate) * 100 + DAY(@CurrentDate))
	                BEGIN
	                    ...
	                    ([DateKey], 
	                     ...
	                     [IsWeekend]
	                           ...
	                           SELECT DateKey = YEAR(@CurrentDate) * 10000 + MONTH(@CurrentDate) * 100 + DAY(@CurrentDate), 
	                                  ...
	                                  Day = DAY(@CurrentDate), 
	                                  [DaySuffix] = CASE
	                                                    WHEN DAY(@CurrentDate) = 1 OR DAY(@CurrentDate) = 21 OR DAY(@CurrentDate) = 31
	                                                    THEN 'st'
	                                                    WHEN DAY(@CurrentDate) = 2 OR DAY(@CurrentDate) = 22
	                                                    THEN 'nd'
	                                                    WHEN DAY(@CurrentDate) = 3 OR DAY(@CurrentDate) = 23
	                                                    THEN 'rd'
	                                                    ELSE 'th'
	                                                END, 
